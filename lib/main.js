...
exports.import = function(config) {
  ...
  // Create a NWGVisual document if the document is a visual type
  function createVisualFromFile(file, remotePath) {
    ...
    return nuxeo.repository()
        ...
        .then((doc) => {
          if (config.verbose) {
            console.log('Created NWGVisual: ' + doc.path);
          }
  ...
  function createDocument(opts) {
    ...
    if (isDirectory) {
      ...
      filesCount++;
      var isVisualDocument = isVisual(opts);
  ...
  function isVisual(file) {
    // Hardcoded list of extensions: should be improved
    var isVisualDocument = ['.png', '.jpg', '.jpeg', '.bmp', '.gif'].includes(path.extname(file.absPath));
    return isVisualDocument;
  } 
